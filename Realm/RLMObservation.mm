...
NSString *RLMObservationInfo::columnName(int64_t col) const noexcept {
    return objectSchema->propertyForTableColumn(realm::ColKey(col)).name;
}
...
void RLMObservationInfo::setRow(realm::Table const& table, int64_t newRow) {
    REALM_ASSERT_DEBUG(!row);
    REALM_ASSERT_DEBUG(objectSchema);
    row = table.get_object(realm::ObjKey(newRow));
    for (auto info : objectSchema->observedObjects) {
        if (info->row && info->row.get_key() == row.get_key()) {
            prev = info;
            next = info->next;
            if (next)
                next->prev = this;
...
void RLMObservationInfo::recordObserver(realm::Obj& objectRow, RLMClassInfo *objectInfo,
    ...
    if (objectRow) {
        ...
        setRow(*objectRow.get_table(), objectRow.get_key().value);
        return;
    }
...
RLMObservationInfo *RLMGetObservationInfo(RLMObservationInfo *info, int64_t row,
    ...
    realm.group.set_cascade_notification_handler([&](realm::Group::CascadeNotification const& cs) {
        ...
        for (auto const& link : cs.links) {
            ...
            for (auto observer : **table) {
                if (!observer->isForRow(link.origin_key.value)) {
                    continue;
                }

                NSString *name = observer->columnName(link.origin_col_key.value);
                ...
                // FIXME: find_first() with offset isn't implemented yet in core
//                size_t start = 0, index;
//                while ((index = linkview.find_first(link.old_target_key, start)) != realm::not_found) {
//                    [c->indexes addIndex:index];
//                    start = index + 1;
//                }
                size_t index = linkview.find_first(link.old_target_key);
...
void forEach(realm::BindingContext::ObserverState const& state, Func&& func) {
    for (auto& change : state.changes) {
        func(change.first, change.second, static_cast<RLMObservationInfo *>(state.info));
    }
}
}
...
void RLMWillChange(std::vector<realm::BindingContext::ObserverState> const& observed,
    ...
    if (!observed.empty()) {
        ...
        for (auto const& o : observed) {
            forEach(o, [&](size_t colKey, auto const& change, RLMObservationInfo *info) {
